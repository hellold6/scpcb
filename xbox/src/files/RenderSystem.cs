...
namespace SCPCB360.Engine
{
    public static class RenderSystem
    {
        ...
        public static void Draw()
        {
            ...
            {
                if (!e.Visible || (e.XnaModel == null && e.RMeshRenderMesh == null)) continue;
        ...
        private static void DrawRMesh(BlitzEntity e, Matrix world)
        {
            _effect.World = world;
            _effect.View = _view;
            _effect.Projection = _projection;
            _effect.DiffuseColor = e.EntityColor.ToVector3();
            _effect.Alpha = e.Alpha;
            ...
            foreach (var surface in e.RMeshRenderMesh.Surfaces)
            {
                _effect.Texture = GetSurfaceTexture(surface);
                _effect.TextureEnabled = _effect.Texture != null;

                foreach (var pass in _effect.CurrentTechnique.Passes)
                {
                    pass.Apply();
                    _gfx.DrawUserIndexedPrimitives(
        ...
        private static Texture2D GetSurfaceTexture(RMeshRenderSurface surface)
        {
            if (surface.Texture != null || string.IsNullOrWhiteSpace(surface.TexturePath))
                return surface.Texture;

            try
            {
                using var stream = File.OpenRead(surface.TexturePath);
                surface.Texture = Texture2D.FromStream(_gfx, stream);
                return surface.Texture;
            }
            catch (System.Exception ex)
            {
                System.Console.WriteLine($"[Render] Failed to load texture '{surface.TexturePath}': {ex.Message}");
                return null;
            }
        }
